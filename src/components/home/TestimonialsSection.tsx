...
const TestimonialsSection = () => {
  ...
  return <section id="testimonials" className="py-24 bg-gradient-to-b from-background to-brand-deeper-purple-100/30 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        <Carousel setApi={setApi} className="w-full max-w-5xl mx-auto">
          <CarouselContent>
            {allTestimonials.map((testimonial, index) => <CarouselItem key={index}>
                <div className="p-1">
                  <TestimonialCard text={testimonial.text} name={testimonial.name} title={testimonial.title} image={testimonial.image} index={index} />
                </div>
              </CarouselItem>)}
          </CarouselContent>
          <div className="flex items-center justify-center mt-8 gap-2">
            <CarouselPrevious className="static translate-y-0 mr-2" />
            {allTestimonials.map((_, index) => <button key={index} onClick={() => api?.scrollTo(index)} className={`w-3 h-3 rounded-full transition-all duration-300 ${activeSlide === index ? "bg-brand-purple" : "bg-gray-300"}`} />)}
...
const TestimonialCard = ({
  ...
  image,
  index
}: TestimonialProps) => <Card className="glassmorphism overflow-hidden hover-scale h-full" style={{
