...
const FeaturesSection = () => {
  ...
  return <section id="features" className="py-24 px-4 sm:px-6 lg:px-8 relative bg-gradient-to-b from-background to-muted/50">      
      <div className="max-w-7xl mx-auto relative">
        <div className="text-center mb-16 animate-fade-in-up">
          <h2 className="text-4xl font-bold mb-6 section-title inline-block pb-4 text-purple-800">Powerful Features</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {featureData.map((feature, index) => <FeatureCard key={feature.title} icon={feature.icon} title={feature.title} description={feature.description} route={feature.route} delay={100 * (index % 3)} isLoggedIn={!!user} />)}
        </div>
      </div>
    </section>;
};
...
const FeatureCard = ({
  ...
  return <CardWrapper>
      <Card className="glassmorphism neon-glow-hover animate-fade-in-up h-full" style={{
      animationDelay: `${delay}ms`
        ...
        <CardContent className="p-6 text-center">
          ...
          {isLoggedIn && <div className="mt-4 text-brand-purple text-sm font-medium">
              Explore Feature →
            </div>}
        </CardContent>
      </Card>
