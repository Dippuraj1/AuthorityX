...
const PricingCard = ({ tier, price, period, description, features, isPopular, delay, textColor }: PricingCardProps) => (
  <Card 
    className={`glassmorphism animate-fade-in-up ${
      isPopular 
        ? "border-brand-purple shadow-glow relative transform hover:-translate-y-2 transition-all duration-500" 
        : "hover:-translate-y-1 transition-all duration-300"
    }`}
    style={{ animationDelay: `${delay}ms` }}
  >
    {isPopular && (
      <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-gradient-to-r from-brand-purple to-brand-light-purple text-white px-6 py-1.5 rounded-full text-sm font-medium shadow-glow">
        Most Popular
      </div>
    )}
