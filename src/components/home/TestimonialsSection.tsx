...
const TestimonialsSection = () => {
  ...
  return <section id="testimonials" className="py-24 bg-gradient-to-b from-background to-brand-deeper-purple/30 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        <div className="flex items-center justify-center mt-10 gap-4">
          <button onClick={() => setActiveTab(prev => prev === "tab1" ? "tab3" : prev === "tab2" ? "tab1" : "tab2")} className="p-3 rounded-full glassmorphism hover:shadow-glow transition-all duration-300">
            <ArrowLeft className="h-5 w-5" />
          </button>
          <div className="flex gap-2">
            {["tab1", "tab2", "tab3"].map(tab => <button key={tab} onClick={() => setActiveTab(tab)} className={`w-3 h-3 rounded-full transition-all duration-300 ${activeTab === tab ? "bg-brand-purple" : "bg-white/20"}`} />)}
          </div>
          <button onClick={() => setActiveTab(prev => prev === "tab1" ? "tab2" : prev === "tab2" ? "tab3" : "tab1")} className="p-3 rounded-full glassmorphism hover:shadow-glow transition-all duration-300">
...
const TestimonialGrid = ({
  testimonials
}: TestimonialGridProps) => <div className="grid gap-6 md:grid-cols-3">
    {testimonials.map((item, index) => <TestimonialCard key={index} {...item} index={index} />)}
  </div>;
const TestimonialCard = ({
  ...
  index: number;
}) => <Card className="glassmorphism overflow-hidden hover-scale" style={{
